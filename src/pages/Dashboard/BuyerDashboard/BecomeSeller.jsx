  ...
  return (
    <div
      className={`top-0 left-0 w-screen h-screen fixed z-10 ${
        sellerModal ? "visible" : "hidden"
      }`}
    >
      <div
        div
        className={`top-0 left-0 relative w-full h-full bg-black bg-opacity-80`}
        id="modal-close"
        onClick={() => setSellerModal(false)}
      ></div>

      <div
        className={`fixed rounded-lg seller-modal-form px-10 pb-10 bg-white z-20 ${
          sellerModal ? "visible" : "hidden"
        }`}
      >
        <IoClose
          size="1.758rem"
          className="absolute z-20 right-1 top-1 text-red-600 cursor-pointer hover:text-red-400"
          onClick={() => setSellerModal(false)}
        />
        <h1 className="text-lg font-bold text-center text-gray-600 mt-8">
          Sell With <span className="text-2xl text-pry-clr">AIGIS</span>{" "}
        </h1>
      ...
      {error.errorState === false && (
        <div className="fixed top-0 left-0 w-screen h-screen flex justify-center items-center z-40">
          <div className="p-8 rounded-lg bg-white font-bold text-green-400 shadow-lg">
            <p>
              Congratulations 🍾🍾, You are now a Seller on{" "}
              <span className="text-xl text-pry-clr">AIGIS</span>
            </p>
            <p className="py-8 text-center font-medium text-red-500">
  ...
  return (
    <div className="flex flex-col w-full">
      <label htmlFor={name} className="relative">
        {text}
        <input
          className="border-gray-300 border rounded-xl font-medium mt-1 px-4 pl-5 py-2 w-full text-black focus:outline-none focus:bg-gray-500 placeholder-purple-300 "
          type={type}
          name={name}
          id={name}
          value={values[name]}
