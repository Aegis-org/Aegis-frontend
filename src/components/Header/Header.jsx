    ...
    <>
      <header className="container mx-auto flex text-center item-center justify-between font-montserrart px-10 py-3 max-w-6xl">
        ...
        <div className="flex mx-5 space-x-4 text-sec-clr nav-links hidden md:inline-flex">
          {headLinks.map((link) => (
            <Link
              to={`/${link.to}`}
              key={link.id}
              className="list-none font-normal item-center justify-center hover:font-bold hover:text-pry-clr pt-2 hover:underline sm:space-x-2"
            >
              {link.name}
            </Link>
          ))}
        ...
        <div className=" flex justify-end hidden md:inline-flex">
          <Link
            to="/"
            className="text-sm text-white font-semibold rounded-full border-2 border-pry-clr px-2 py-1 mx-6 bg-pry-clr"
          >
            Create An Account
          </Link>
          <Link
            to="/login"
            className="px-4 drop-shadow-md py-1 text-sm font-semibold rounded-full border-2 border-pry-clr bg-white hover:text-white hover:bg-pry-clr"
          >
            Log in
          </Link>
        </div>
